<template>
    <div class="container-fluid">
      <h4 class="mb-3">Edit Polygon Lahan - Metode: Tambah Titik Manual</h4>
        

      <h5 class="mb-3">Polygon Lama:</h5>
<div id="mapOld" class="mb-4" style="height: 300px; border-radius: 12px; overflow: hidden"></div>

<h5 class="mb-3">Polygon Baru:</h5>
      ...
      <div class="row align-items-center gy-2 mb-2">
        ...
        <div class="col-12 col-md-8">
          <div class="d-flex flex-wrap gap-2 justify-content-md-end">
            ...
            <button class="btn btn-success" :disabled="!area || path.length < 3" @click="submitEdit">
  ...
  aktifBasemaps.forEach((b) => {
    ...
    // Clone basemap untuk masing-masing map
    this.baseLayersMap[b.judul] = L.tileLayer(url, options);
    this.baseLayersMapOld[b.judul] = L.tileLayer(url, options);
  });
  ...
  this.initMap();
  this.initMapOld(); // ← Tambahkan ini untuk peta polygon lama
},
initMapOld() {
  ...
  L.control.layers(this.baseLayersMapOld).addTo(this.mapOld);
  ...
  this.mapOld.fitBounds(polygonLayer.getBounds());
}, 
      initMap() {
        navigator.geolocation.getCurrentPosition(
          (pos) => {
            ...
            const defaultLayer = Object.values(this.baseLayersMap)[0];
            this.map = L.map("map", {
              center: [lat, lng],
              zoom: 18,
              layers: [defaultLayer],
            ...
            L.control.layers(this.baseLayersMap).addTo(this.map);
